...
export default function Emergency() {
  ...
  const [gpsCoords, setGpsCoords] = useState({ lat: 24.2502, lng: 89.9167 }) // Default to Tangail coordinates
      ...
      <div className="glass-card animate-float-up" style={{ width: '100%', maxWidth: 520, padding: 32, border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 28 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div>
            ...
            <select
              id="simulate-region"
              value={gpsCoords.lat === 24.2502 && gpsCoords.lng === 89.9167 ? 'tangail' : gpsCoords.lat === 23.8103 && gpsCoords.lng === 90.4125 ? 'dhaka' : gpsCoords.lat === 24.8917 && gpsCoords.lng === 91.8688 ? 'sylhet' : 'gps'}
              onChange={(e) => {
                const val = e.target.value
                if (val === 'tangail') {
                  setGpsCoords({ lat: 24.2502, lng: 89.9167 })
                  setGpsError(null)
                } else if (val === 'dhaka') {
                  setGpsCoords({ lat: 23.8103, lng: 90.4125 })
                  setGpsError(null)
                } else if (val === 'sylhet') {
                  setGpsCoords({ lat: 24.8917, lng: 91.8688 })
                  setGpsError(null)
                } else {
                  acquireLocation(true)
                }
            ...
            >
              <option value="tangail" style={{ background: 'var(--bg-secondary)', color: '#fff' }}>Tangail (Accident Site)</option>
              <option value="dhaka" style={{ background: 'var(--bg-secondary)', color: '#fff' }}>Dhaka (Capital District)</option>
              <option value="sylhet" style={{ background: 'var(--bg-secondary)', color: '#fff' }}>Sylhet (Northeast Highway)</option>
