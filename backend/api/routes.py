...
async def rag_query(payload: RAGQuerySchema):
    ...
    Falls back to hardcoded offline protocols if ChromaDB is not seeded.
    """
    print(f"[RAG QUERY] '{payload.query}' | k={payload.k}")
    try:
        from rag.retrieval.searcher import retrieve_guidelines
        results = retrieve_guidelines(payload.query, k=payload.k)
        return {
            "status": "success",
            "query": payload.query,
            "results": results,
        ...
        from rag.retrieval.searcher import get_offline_fallback
        ...
        return {
            ...
            "warning": "ChromaDB unavailable — serving cached emergency protocols"
        }
